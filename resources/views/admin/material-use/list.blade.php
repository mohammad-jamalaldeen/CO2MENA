...
@endsection
@section('content')
@if (!adminPermissionCheck('material-use.create'))
@php
...
@endif
    <div class="table-header {{$classlist}}">
        <div class="row align-items-center" {{$display}}>
            ...
            <div class="col-md-7 col-12">
                <div class="dw-header">
                    <a class="createsheet-btn" href="{{ route('material-use.create') }}" title="create">
                        CREATE Material Use
                    </a>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12">
                    <ul class="new-table-filter">
                        <li>
                            <select name="activity" id="activity" title="Activity">
    ...
    <div class="datasheet-table tb-filter-inner {{$classlist}}">
        <div class="responsive-table">
            <table id="manage-material-use" class="table custome-datatable manage-customer-table display"
                 >
                <thead>
                    <tr>
                        <th class="mw-140">Activity</th>
                        <th class="mw-140">Waste Type</th>
...
@section('footer_scripts')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $("#manage-material-use").DataTable({
                processing: true,
                serverSide: true,
                searching: true,
                lengthChange: true,
                ...
                "initComplete": function (settings, json) {  
                    $("#manage-material-use").wrap("<div class='table-main-wrap manage-material-use-wrap'></div>");            
                },
                language: {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var html = "";
                            var id = full.id;
                            var current_url_show = "{{ url('admin/material-use/show') }}/" + id;
                            var current_url_edit = "{{ url('admin/material-use/edit') }}/" + id;
                            @if(adminMultiplePermissionCheck('material-use', ['show', 'edit', 'delete']) > 0)
                                html +=
                                ...
                                @if (adminPermissionCheck('material-use.show'))
                                    html += '<li><a class="dropdown-item" href="' +current_url_show +'">View</a></li>';
                                @endif
                                @if (adminPermissionCheck('material-use.edit'))
                                    html += '<li><a class="dropdown-item" href="' +current_url_edit +'">Edit</a></li>';
                                @endif
                                @if (adminPermissionCheck('material-use.delete'))
                                    html += '<li><a class="dropdown-item deletematerialuse" href="#" data-id =' +full.id + '>Delete</a></li>';
                                @endif
                                html += '</ul></div>';
                            @else
                                html += '-';
                ...
                "drawCallback": function(settings) {
                    ...
                    $("#manage-material-use_previous").html(
                        '<img src="{{ asset('assets/images/back-arrow.svg') }}" alt="back-icon">');
                    $("#manage-material-use_next").html(
        ...
        function resetValue() {
            ...
            $('select[name="activity"]').selectpicker('refresh');
            $('#manage-material-use').DataTable().draw();
        }

        $(document).on('click', ".deletematerialuse", function() {
            var id = $(this).attr('data-id');
            $(".deleterecordbtn").attr('data-id', id);
            $(".delete-modal-title").text("Material Uses Delete");
            $(".delete-modal-body").html("<p>Are you sure? you want to delete this record.</p>");
            $("#deleterecordModel").modal('show');
        });
        $(".deleterecordbtn").click(function() {
            var id = $(this).attr('data-id');
            var current_object = "{{ url('admin/material-use/delete') }}" + '/' + id;
