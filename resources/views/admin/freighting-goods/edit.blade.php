...
@section('content')
    ...
    <div class="customer-support">
        <form class="input-form" id="bussiness-edit" method="POST"
            action="{{ route('freighting-goodsgvh.update', $fgedit->id) }}">
            @csrf
            <div class="row">
                        ...
                        <select name="vehicle" placeholder="Select Vehicle" id="vehicle" data-live-search="true">
                            @foreach (\App\Models\FreightingGoodsVansHgvs::VEHICLE as $value)
                                <option value="{{ $value }}" {{ old('vehicle') == $value ? 'selected' : '' }}
                                    {{ optional($fgedit)->vehicle === $value ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                        @error('vehicles')
                ...
                <div class="col-md-4 col-sm-6 col-12">
                    <div class="form-group">
                        ...
                        <select name="fuel" placeholder="Select Fuel" id="fuel" data-live-search="true">
                            @foreach (\App\Models\FreightingGoodsVansHgvs::FUEL as $value)
                                <option value="{{ $value }}" {{ old('fuel') == $value ? 'selected' : '' }}
                                    {{ optional($fgedit)->fuel === $value ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                        @error('vehicles')
