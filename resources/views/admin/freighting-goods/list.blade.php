...
@endsection
@section('content')
@if (!adminPermissionCheck('freighting-goodsgvh.create'))
    @php
...
@endif
    <div class="table-header {{$classlist}}">
        <div class="row align-items-center" {{$display}}>
            ...
            <div class="col-md-7 col-12">
                <div class="dw-header">
                    <a class="createsheet-btn" href="{{ route('freighting-goodsgvh.create') }}" title="create">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <ul class="new-table-filter">
                        ...
                        <li>
                            <select class="col-md-4 col-12" name="fuel" title="Fuel" id="getfuel">
    ...
    <div class="datasheet-table tb-filter-inner {{$classlist}}">
        <div class="responsive-table">
            <table id="fg" class="table custome-datatable manage-customer-table display"  >
                <thead>
                    <tr>
                        {{-- <th>Row ID</th> --}}
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $("#fg").DataTable({
                ...
                order: [],
                ajax: {
                    url: "{{ route('freighting-goodsgvh.index') }}",
                    data: function(d) {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var fg_show_url = "{{ url('admin/freighting-goodsgvh/show') }}" + '/' +
                                full.id;
                            var fg_edit_url = "{{ url('admin/freighting-goodsgvh/edit') }}" + '/' +
                                full.id;
                            var html = "";
                            @if(adminMultiplePermissionCheck('freighting-goodsgvh', ['show', 'edit', 'delete']) > 0)
                                html +=
                                ...
                                @if(adminPermissionCheck('freighting-goodsgvh.show'))
                                    html += '<li><a class="dropdown-item" href="' +fg_show_url +'">View</a></li>';
                                @endif
                                @if(adminPermissionCheck('freighting-goodsgvh.edit'))
                                    html += '<li><a class="dropdown-item" href="' +fg_edit_url +'">Edit</a></li>';
                                @endif
                                @if(adminPermissionCheck('freighting-goodsgvh.delete'))
            ...
            $('#getvehicle, #getfuel').change(function() {
                $("#fg").DataTable().draw();
            });
            ...
            $(document).on('click', ".deleteemployeescommuting", function() {
                ...
                $(".delete-modal-title").text("Freighting Good Vans And Hgvs");
                $(".delete-modal-body").html("<p>Are you sure? you want to delete this record.</p>");
                $("#deleterecordModel").modal('show');
            });
            $(".deleterecordbtn").click(function() {
                var id = $(this).attr('data-id');
                var current_object = "{{ url('admin/freighting-goodsgvh/delete') }}" + '/' + id;
            ...
            $('select[name="fuel"]').on('change', function() {
                table.ajax.reload();
            });
        });

        function resetValue() {
            $('select[name="vehicle"], select[name="fuel"]').val(null);
            $('select[name="vehicle"], select[name="fuel"]').selectpicker('refresh');
            $('#fg').DataTable().draw();
        }
