...
@section('content')
    ...
    <div class="customer-support">
        <form class="input-form" id="bussiness-edit" method="POST"
            action="{{ route('bussinesstravel.update', $bussinessedit->id) }}">
            @csrf
            <div class="row">
                {{-- <div class="col-md-4 col-sm-6 col-12">
                    <div class="form-group">
                        <label>Row ID</label>
                        <input type="number" name="row_id" value="{{ $bussinessedit->row_id ?? old('row_id') }}"
                    ...
                    <label for="vehicles">Vehicle <span class="mandatory-field">*</span></label>
                        <select name="vehicles" placeholder="Select Vehicle" id="vehicles" data-live-search="true">
                            @foreach (\App\Models\BusinessTravels::VEHICLE as $value)
                                <option value="{{ $value }}"
                                    {{ optional($bussinessedit)->vehicles === $value ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                        @error('vehicles')
                    ...
                    <label for="type">Type <span class="mandatory-field">*</span></label>
                        <select placeholder="Select Type" name="type" id="type" data-live-search="true">
                            @foreach (\App\Models\BusinessTravels::TYPE as $value)
                                <option value="{{ $value }}"
                                    {{ optional($bussinessedit)->type === $value ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                        @error('type')
                    ...
                    <label for="fuel">Fuel <span class="mandatory-field">*</span></label>
                        <select placeholder="Select Fuel" name="fuel" id="fuel" data-live-search="true">
                            @foreach (\App\Models\BusinessTravels::FUEL as $value)
                                <option value="{{ $value }}"
                                    {{ optional($bussinessedit)->fuel === $value ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                        @error('fuel')
                ...
                <div class="col-md-4 col-sm-6 col-12">
                    ...
                    <label for="unit">Unit <span class="mandatory-field">*</span></label>
                        <select placeholder="Select Unit" name="unit" id="unit" data-live-search="true">
                            ...
                            <option value="{{ $value }}"
                            {{ optional($bussinessedit)->unit === $value ? 'selected' : '' }}>
                            {{ $value }}</option>
                            @endforeach
                        </select>
                        @error('unit')
                ...
                <div class="col-md-4 col-sm-6 col-12">
                    <div class="form-group">
                        ...
                        <input type="text" name="factors" id="factors" value="{{ $bussinessedit->factors ?? old('factors') }}"
                ...
                {{-- <div class="col-md-4 col-sm-6 col-12">
                    <div class="form-group">
                        <label>Formula</label>
                        <input type="text" name="formula" value="{{ $bussinessedit->formula ?? old('formula') }}"
